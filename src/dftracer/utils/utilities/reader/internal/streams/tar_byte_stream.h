 ...
 * files. Presents concatenated file contents without TAR headers.
 */
class TarByteStream : public TarStream {
   ...
   public:
    ...
    void initialize(const std::string &tar_gz_path, std::size_t start_bytes,
        ...
        DFTRACER_UTILS_LOG_DEBUG(
            "TarByteStream::initialize - completed, current_position_=%zu",
            current_position_);
    }
    ...
    coro::CoroTask<std::size_t> read_async(char *buffer,
        ...
        if (!decompression_initialized_) {
            throw ReaderError(ReaderError::INITIALIZATION_ERROR,
                              "TAR streaming session not properly initialized");
        }
        ...
        if (total_bytes_read == 0 || current_position_ >= target_end_bytes_) {
            is_finished_ = true;
        }

        DFTRACER_UTILS_LOG_DEBUG(
            "TarByteStream::stream - total bytes read: %zu (position: %zu / "
            "%zu)",
