...
class TestDaskIntegration:
    ...
    def test_parallel_reader_operations(self):
        ...
        with Environment(lines=2000) as env:
            ...
            assert len(successful_results) == len(results), (
                f"Some operations failed: {failed_results}"
            )
    ...
    def test_multiple_batch_sizes_no_duplication(self):
        ...
        with Environment(lines=200) as env:  # More records for better boundary testing
            ...
            assert expected_count == env.lines, (
                f"Reference data mismatch: got {expected_count} records, environment has {env.lines} lines"
            )
            ...
            for batch_size in batch_sizes:
                ...
                assert len(duplicates) == 0, (
                    f"Batch size {batch_size}: Found {len(duplicates)} duplicates!"
                )
                ...
                assert len(missing_names) == 0, (
                    f"Batch size {batch_size}: Missing {len(missing_names)} records: {sorted(missing_names)}"
                )
                assert len(extra_names) == 0, (
                    f"Batch size {batch_size}: Found {len(extra_names)} extra records: {sorted(extra_names)}"
                )
                assert actual_count == expected_count, (
                    f"Batch size {batch_size}: Expected {expected_count} records, got {actual_count}"
                )
                assert actual_count == env.lines, (
                    f"Batch size {batch_size}: Expected {env.lines} env.lines, got {actual_count}"
                )

                print(
                    f"  Batch size {batch_size // 1024}KB: {actual_count} records, no duplicates, complete recovery"
                )
    ...
    def test_boundary_edge_cases(self):
        ...
        with Environment(lines=100) as env:
            ...
            print(
                f"Created {len(batches)} batches, expecting {expected_count} total records (env.lines: {env.lines})"
            )
